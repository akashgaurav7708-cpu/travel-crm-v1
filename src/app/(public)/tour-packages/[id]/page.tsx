...
import { Star, Clock, MapPin, CheckCircle2, Phone, MessageCircle } from 'lucide-react';
...
export default function PackageDetail() {
  ...
  return (
    <div className="pt-24 pb-24 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-slate-100 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-2">
            <div className="mb-10">
               <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-1 text-brand-gold">
                    ...
                    <Star size={14} fill="#D4AF37" stroke="none" />
                    <Star size={14} fill="#D4AF37" stroke="none" />
                    <span className="text-slate-900 dark:text-slate-300 text-xs font-bold ml-1">(4.9/5 Rating)</span>
                  </div>
               </div>
               <h1 className="text-4xl md:text-5xl font-serif font-black text-brand-navy dark:text-white tracking-tight mb-6">{pkg.name}</h1>
               <div className="flex flex-wrap gap-6 text-sm text-slate-500 font-medium font-sans">
            ...
            <div className="prose prose-slate max-w-none mb-16 font-sans">
               <h3 className="text-2xl font-serif font-black text-brand-navy dark:text-white mb-4">About This Package</h3>
               <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-lg italic border-l-4 border-brand-gold pl-6">&quot;{pkg.description}&quot;</p>
            </div>

            <div className="mb-16 font-sans">
               <h3 className="text-2xl font-serif font-black text-brand-navy dark:text-white mb-8">Trip Itinerary</h3>
                  ...
                  {pkg.itinerary.map((day) => (
                    <div key={day.day} className="flex gap-8 group">
                       <div className="flex flex-col items-center">
                          ...
                          <div className="w-0.5 flex-grow bg-slate-100 dark:bg-zinc-800 my-2"></div>
                       </div>
                       <div className="pb-8">
                          <h4 className="text-xl font-serif font-black text-brand-navy dark:text-white mb-2">Day {day.day}: {day.title}</h4>
                          <p className="text-slate-500 dark:text-slate-400 leading-relaxed">{day.desc}</p>
                       </div>
                    </div>
                  ))}
               </div>
            ...
            <div className="bg-slate-50 dark:bg-zinc-900 rounded-[2.5rem] p-10 border dark:border-zinc-800 font-sans">
               <h3 className="text-2xl font-serif font-black text-brand-navy dark:text-white mb-8">What&apos;s Included</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {pkg.inclusions.map((item) => (
                    <div key={item} className="flex items-center gap-3 text-slate-600 dark:text-slate-400 font-bold">
          ...
          <div className="lg:col-span-1">
             <div className="sticky top-32 bg-white dark:bg-zinc-900 rounded-[2.5rem] border dark:border-zinc-800 shadow-2xl p-8 md:p-10 font-sans">
                <div className="mb-8">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Package Cost</p>
                   <div className="flex items-baseline gap-2">
                      <span className="text-4xl font-serif font-black text-brand-navy dark:text-white">₹{pkg.price}</span>
                ...
                <div className="space-y-4 mb-8">
                   <div className="bg-slate-50 dark:bg-zinc-800/40 p-4 rounded-xl border dark:border-zinc-800">
                      <p className="text-[9px] font-black uppercase text-slate-400 mb-1">Travel Date</p>
                      <p className="text-sm font-black text-brand-navy dark:text-slate-200">Select Date</p>
                   </div>
                   <div className="bg-slate-50 dark:bg-zinc-800/40 p-4 rounded-xl border dark:border-zinc-800">
                      <p className="text-[9px] font-black uppercase text-slate-400 mb-1">Travelers</p>
                      <p className="text-sm font-black text-brand-navy dark:text-slate-200">2 Adults, 0 Children</p>
                   </div>
                </div>

                <div className="space-y-3">
                   <button className="w-full bg-brand-navy text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-brand-gold hover:text-brand-navy transition-all shadow-xl">
                      Book This Package
                   </button>
                   <a href="tel:+916006070550" className="w-full border-2 border-brand-navy text-brand-navy dark:border-zinc-700 dark:text-slate-300 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all flex items-center justify-center gap-2">
                      <Phone size={16} /> Call Expert
                   </a>
                   <a href="https://wa.me/916006070550" target="_blank" rel="noopener noreferrer" className="w-full bg-brand-emerald text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-emerald-600 transition-all shadow-xl flex items-center justify-center gap-2">
